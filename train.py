...
def test(model, path, dataset):
    ...
    for i in range(num1):
        ...
        res, res1, res2, res3, res4, _ = model(image)
        # eval Dice
...
def train(train_loader, model, optimizer, epoch, test_path):
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            # ---- forward ----
            P1, P2, P3, P4, P5, out_edge = model(images)
            ...
            loss_out = loss_P1 + loss_P2 + loss_P3 + loss_P4 + loss_P5
    ...
    if (epoch + 1) % 1 == 0 and epoch >= 50:
        meandice = test(model, test_path, 'var')
        writer.add_scalar('test_loss', meandice, epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_path', type=str,
                        default='/root/autodl-tmp/TrainDatasetEdges/',
                        help='path to train dataset')

    parser.add_argument('--test_path', type=str,
                        default='/root/autodl-tmp/TestDataset/',
                        help='path to testing Kvasir dataset')

    parser.add_argument('--train_save', type=str,
                        default='./model_pth/polyp9-7/')
    ...
    model = torch.nn.DataParallel(BSNet(), device_ids=[0]).cuda()
    ...
    # print("amp后的模型",model.backbone)
    writer = SummaryWriter('figure')
    image_root = '{}/images/'.format(opt.train_path)
    gt_root = '{}/masks/'.format(opt.train_path)
    eg_root = '{}/edges/'.format(opt.train_path)

    train_loader = get_loader(image_root, gt_root, eg_root, batchsize=opt.batchsize, trainsize=opt.trainsize,
